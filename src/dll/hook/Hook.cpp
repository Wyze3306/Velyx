...
bool Hook::create(void* detour, void** original) {
    if (target_ == 0) {
        Log::warn(kLog, "{} : adresse nulle, ignoré", name_);
        return false;
    }
...
bool Hook::createAt(void* address, void* detour, void** original) {
    ...
    if (status != MH_OK) {
        Log::error(kLog, "{} : MH_CreateHook a échoué ({})", name_, MH_StatusToString(status));
        return false;
    }

    const MH_STATUS queued = MH_QueueEnableHook(address);
    if (queued != MH_OK) {
        Log::error(kLog, "{} : MH_QueueEnableHook a échoué ({})", name_, MH_StatusToString(queued));
